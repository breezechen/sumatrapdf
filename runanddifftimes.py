...
# Usage:
# runanddifftimes.py <exe-one> <exe-two> <pdf-file> [<count>]
...
def usage_and_exit():
  print "Usage: runanddifftimes.py <exe-one> <exe-two> <pdf-file> [<count>]"
  sys.exit(1)
...
def run(exe, pdf):
  print "executing '%s -preview -timings %s'" % (exe, pdf)
  return Popen([exe, "-preview", "-timings", pdf], stdout=PIPE).communicate()[0]
