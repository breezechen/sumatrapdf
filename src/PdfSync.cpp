...
UINT CreateSynchronizer(LPCTSTR pdffilename, Synchronizer **sync)
{
    ...
    if (n>u && _tcsicmp(pdffilename+(n-u), PDF_EXTENSION) == 0 ) {
        ...
        DBG_OUT_T(_T("Bad PDF filename! (%s)\n"), pdffilename);
        return PDFSYNCERR_INVALID_ARGUMENT;
    }
}
...
FILE *Pdfsync::opensyncfile()
{
    ...
    if (NULL == fp) {
        DBG_OUT_T(_T("The syncfile %s cannot be opened\n"), syncfilepath);
        return NULL;
    }
    return fp;
}
...
UINT Pdfsync::source_to_pdf(LPCTSTR srcfilename, UINT line, UINT col, UINT *page, vector<RectI> &rects)
{
    ...
    if (ret!=PDFSYNCERR_SUCCESS || found_records.size() == 0 ) {
        DBG_OUT_T(_T("source->pdf: %s:%u -> no record found, error:%u\n"), srcfilename, line, ret);
        fclose(fp);
        return ret;
    }
                ...
                while ((c = fgetc(fp))=='p' && !feof(fp)) {
                    ...
                    if (recordNumber == record) {
                        *page = (UINT)sheet;
                        ...
                        rc.dy = MARK_SIZE;
                        rects.push_back(rc);
                        DBG_OUT_T(_T("source->pdf: %s:%u -> record:%u -> page:%u, x:%u, y:%u\n"),
